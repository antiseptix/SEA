...
int main(int argc, char** argv)
{
  ...
  //ouverture du descripteur écriture output file | si non existant le créer
  descOutput = open(bin_output_param,O_CREAT|O_RDWR,00777);
  
  //gestion des erreurs de mauvais fichiers sortie (test de perror())
  if(errno != 0)
  {
	  perror("Erreur fichier de sortie :");
	  return EXIT_FAILURE;
  }
  
  //itération lecture d'un block et ecriture dans le second descripteur
  int nbRead = 0;
